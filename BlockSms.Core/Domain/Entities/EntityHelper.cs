...
namespace BlockSms.Core.Domain.Entities
{
    public static class EntityHelper
    {
        ...
        {
            if (EqualityComparer<TKey>.Default.Equals(entity.KeyId, default))
            {
                return true;
            }
            ...
            {
                return Convert.ToInt32(entity.KeyId) <= 0;
            }

            if (typeof(TKey) == typeof(long))
            {
                return Convert.ToInt64(entity.KeyId) <= 0;
            }
        ...
        public static Type FindPrimaryKeyType([NotNull] Type entityType)
        {
            ...
            {
                throw new EPTException($"Given {nameof(entityType)} is not an entity. It should implement {typeof(IEntity).AssemblyQualifiedName}!");
            }
            ...
            where TEntity : IEntity<TKey>
        {
            ...
            var lambdaBody = Expression.Equal(
                Expression.PropertyOrField(lambdaParam, nameof(Entity<TKey>.KeyId)),
